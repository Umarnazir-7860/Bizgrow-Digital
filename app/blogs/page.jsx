  ...
  if (posts.length === 0) {
    return (
      <div className="text-center py-20">
        <h2 className="text-2xl font-bold text-gray-600">No blogs found at the moment.</h2>
        <Link href="/" className="text-[#B54118] underline mt-4 inline-block">Back to Home</Link>
      </div>
    );
  }

  return (
    <div className="max-w-7xl mx-auto px-4 py-16 bg-[#FDFCF9]">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
        {posts.map((post) => (
          <article key={post.id} className="group bg-white rounded-2xl shadow-sm hover:shadow-xl transition-all duration-300 border border-gray-100 flex flex-col overflow-hidden">
            ...
            <div className="p-8 flex flex-col flex-grow">
              <h2 className="text-xl font-extrabold text-[#12066a] mb-4 line-clamp-2 leading-tight group-hover:text-[#B54118] transition-colors"
      ...
      {totalPages > 1 && (
        <nav className="flex justify-center items-center gap-8 mt-20 border-t border-gray-100 pt-10">
          ...
          <span className="font-mono font-bold text-lg text-gray-400">
            {currentPage.toString().padStart(2, '0')} <span className="text-gray-200 mx-2">/</span> {totalPages.toString().padStart(2, '0')}
          </span>
