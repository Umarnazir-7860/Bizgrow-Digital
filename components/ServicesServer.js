  ...
  return (
    <section ref={container} className="relative w-full h-[300vh] md:h-[500vh] bg-[#F2E8D5]">
      <div className="sticky top-0 h-screen w-full flex flex-col items-center justify-center overflow-hidden px-4">
        ...
        {/* 🔹 FIX: Heading aur Paragraph ko wapas center kiya */}
        <div className="text-center z-10 w-full flex flex-col items-center">
          <FadeIn direction="up">
            <h1 className="text-4xl md:text-6xl text-[#B54118] font-bold uppercase tracking-tighter leading-none">
        ...
        {/* 🔹 FIX: Cards container ki height mobile ke liye behtar ki taaki image cutoff na ho */}
        ...
        {/* 🔹 FIX: Button position and spacing */}
        <div className="mt-12 z-10">
          <FadeIn direction="up" delay={0.4}>
            <Link href="/services">
              <button className="px-8 py-4 bg-[#B54118] text-white text-base font-semibold rounded-xl hover:scale-105 transition-all shadow-lg uppercase tracking-wider">
                View All Services →
              </button>
            </Link>
          </FadeIn>
...
// Card component same rahega bas classes aur transform ko stabilize kiya hai
  ...
  return (
    ...
    >
      ...
      <div className="w-full md:w-1/2 text-center md:text-left">
        <span className="text-[#B54118] font-bold text-xs uppercase tracking-widest opacity-60">Service 0{i + 1}</span>
        <h2 className="text-2xl md:text-4xl text-[#B54118] font-black mt-1 leading-tight uppercase">{title}</h2>
        <p className="text-gray-700 dark:text-gray-300 text-sm md:text-lg mt-2 md:mt-4">{desc}</p>
      </div>
    </motion.div>
  );
}