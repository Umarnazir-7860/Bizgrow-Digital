...
export default function ServicesSection() {
  ...
  return (
    <section ref={container} className="relative w-full h-[250vh] md:h-[500vh] bg-[#F2E8D5]">
      {/* 🔹 FIX: h-[100dvh] use kiya taaki mobile browsers ka address bar layout na bigaray */}
      <div
        className="sticky top-0 h-[70vh]  md:h-[100dvh] w-full flex flex-col items-center 
      justify-start md:justify-center overflow-hidden px-4"
      >
        {/* 🔹 FIX: Spacing kam ki taaki heading na chupay (pt-12 for mobile) */}
        <div className="text-center mt-20 pt-12 md:pt-12  md:pb-12 z-10 w-full shrink-0">
          {/* <FadeIn direction="up" className="hidden md:block"> */}
          <h1 className="text-4xl
           sm:mt-10 lg:-mt-12 sm:text-4xl md:text-6xl text-[#B54118] font-bold uppercase tracking-tighter leading-none">
          ...
          <p
            className="text-black  mt-3 md:mt-2 my-12 text-xl md:text-lg font-medium 
        ...
        <div className="relative w-full mt-6 max-w-5xl h-[45vh] md:h-[60vh] flex items-center justify-center">
        ...
        <div className="mb-8 md:mb-2 md:mt-12 z-10">
          <Link href="/services">
            <button className="px-8 py-4 mt-20 md:px-8 md:py-4 bg-[#B54118] text-white 
            text-[15px] md:text-base font-semibold rounded-xl hover:scale-105 transition-all
...
function Card({ title, img, desc, i, progress, range, isFirst, isLast }) {
  ...
  return (
    <motion.div
      ...
      className="absolute w-[92%] md:w-full max-w-3xl shadow-xl rounded-[1.2rem] md:rounded-[2rem]
       bg-white dark:bg-[#000B25] p-3 md:p-10 border border-gray-100 flex flex-col md:flex-row
    ...
    >
      <div className="w-full md:w-1/2 overflow-hidden rounded-lg md:rounded-2xl shrink-0">
        <Image
          ...
          className="object-cover h-[140px] sm:h-[180px] md:h-[260px] w-full"
          priority={i === 0}
        />
      </div>
